...
public class RequestEnchantItem extends L2GameClientPacket
{
	...
	public static final Map<Integer, Long> _enchantReuse = new ConcurrentHashMap<>();
	private static final long ENCHANT_REUSE = Config.ENCHANT_REUSE * 1000L; // 7 Seconds
	...
	protected void runImpl()
	{
		...
		// Enchant reuse
		if (player.getAccessLevel() < 1 && _enchantReuse.containsKey(player.getObjectId()))
		{
			if (_enchantReuse.get(player.getObjectId()) > currentTime)
			{
				long reuse = (_enchantReuse.get(player.getObjectId()) - currentTime) / 1000;
				if(Config.ENCHANT_SPAM_LOGS)
				{
					System.out.println("Player: " + player.getName() + " enchant spammer");
				}
				player.sendMessage("Enchant Reuse: " + reuse + " seconds" );
				if(Config.ENCHANT_SPAM_KICK)
				{
					player.kick();
				}
				return;
		...
		try
		{
			...
			if (Rnd.chance(chance))
			{
				...
				_enchantReuse.put(player.getObjectId(), currentTime + ENCHANT_REUSE);
